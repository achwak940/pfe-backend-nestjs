...
export class EnqueteController {
  ...
  @Get('/findbyuser/:id')
  findenquetebyUser(@Param('id') id: string) {
  ...
  changeTypeParticipation(@Param('id') id: string, @Body('typeParticipation') type: TypeParticipation) {
  ...
  async submitEnquete(
    ...
    // Support des deux formats: { answers: [] } ou { reponses: [] }
    ...
    // Normaliser les données pour le service
    ...
    return this.enqueteService.submitEnquete(+id, normalizedAnswers);
  }
}